...
class IOManagement:
    ...
    def read_matrixes_from_setup(conn: sqlite3.Connection, id_setup, train_perc, dataset):
        ...
        try:
            ...
            the_train, the_test = IOManagement.__split_dsentries(entry_list, train_perc)
            the_train_mx = IOManagement.datalist_to_datamatrix(the_train)
            the_test_mx = IOManagement.datalist_to_datamatrix(the_test)
            the_train_exp = IOManagement.get_expected_array(the_train)
            the_test_exp = IOManagement.get_expected_array(the_test)

            the_matrix = IOManagement.datalist_to_datamatrix(datalist)
    ...
    @staticmethod
    def get_expected_array(dsentries):
        expected_list = []
        for entry in dsentries:
            entry: DsEntry
    ...
    @staticmethod
    def datalist_to_datamatrix(datalist):
    ...
    def __load_dataset_files_IMBD():
        entries_socal_app = IOManagement.__read_partial_set(RUTA_BASE + 'result-IMDB-SOCAL.txt')
        entries_svr_app = IOManagement.__read_partial_set(RUTA_BASE + 'result-IMDB-SVR62.txt')
        entries_comments = IOManagement.__read_partial_set(RUTA_BASE + 'revs_imdb.txt')
        complete_results = IOManagement.__assign_class(IOManagement.__join_result_sets(entries_socal_app, entries_svr_app))
        return IOManagement.join_partial_set_entries(complete_results, entries_comments, DATASET_IMDB)

    @staticmethod
    def __load_dataset_files_APP():
        entries_socal_app = IOManagement.__read_partial_set(RUTA_BASE + 'result-APP-SOCAL.txt')
        entries_svr_app = IOManagement.__read_partial_set(RUTA_BASE + 'result-APP-SVR62.txt')
        entries_comments = IOManagement.__read_partial_set(RUTA_BASE + 'revs_APP.txt')
        complete_results = IOManagement.__assign_class(IOManagement.__join_result_sets(entries_socal_app, entries_svr_app))
        return IOManagement.join_partial_set_entries(complete_results, entries_comments, DATASET_APP)

    @staticmethod
    def load_dataset_from_files(dataset):
        if dataset == DATASET_APP:
            return IOManagement.__load_dataset_files_APP()
        elif dataset == DATASET_IMDB:
            return IOManagement.__load_dataset_files_IMBD()

    @staticmethod
    def load_dataset(conn: sqlite3.Connection, dataset, light_weigth=False):
        user_cases = IOManagement.load_dataset_from_db(conn, dataset, light_weigth)
        if not user_cases:
            user_cases = IOManagement.load_dataset_from_files(dataset)
    ...
    def __join_result_sets(set1, set2):
        ...
        if IOManagement.__check_set_algorithm(set1, TAG_SVR):
            set_svr = set1
        elif IOManagement.__check_set_algorithm(set2, TAG_SVR):
            set_svr = set2
        else:
            raise Exception("No SVR set in join")

        if IOManagement.__check_set_algorithm(set1, TAG_SOCAL):
            set_socal = set1
        elif IOManagement.__check_set_algorithm(set2, TAG_SOCAL):
            set_socal = set2
        else:
        ...
        for sockey in set_socal:
            ...
            svr_partner = IOManagement.__get_entry(sockey, set_svr)
            if svr_partner:
